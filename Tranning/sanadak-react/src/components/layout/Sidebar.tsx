...
const navGroups = [
  {
    ...
    items: [
      { role: 'recruiter' as Role, labelKey: 'roleRecruiterTitle', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { role: 'student' as Role, labelKey: 'studentPortal', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
    ],
  },
  {
    labelKey: 'navFinance',
...
export default function Sidebar({ onOpenApplicationModal }: SidebarProps) {
    ...
    <aside id="sidebar" className="w-72 bg-white border-e border-gray-200 flex-shrink-0 flex flex-col z-20 transition-all duration-300 h-screen shadow-lg">
      ...
      <div className="p-6 pb-4 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] font-bold text-brand-600 mt-1">منصة التوظيف والتعليم</p>
          </div>
        </div>
      </div>
        ...
        {navGroups.map(group => (
          <div key={group.labelKey} className="space-y-2">
            ...
            <div className="space-y-1">
              ...
              {/* Application form link under recruitment group */}
              {group.labelKey === 'navRecruitment' && (
                <button
                  onClick={onOpenApplicationModal}
                  className="sidebar-link w-full flex items-center px-4 py-3 rounded-xl transition-all text-sm font-bold text-zinc-600 hover:text-brand-600 hover:bg-brand-50/50"
                >
                  <div className="flex items-center gap-3">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                    <span>{t('newApplicationTitle')}</span>
                  </div>
                </button>
              )}
            </div>
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center gap-3 px-2 py-1">
          <div className="w-9 h-9 rounded-xl bg-brand-50 border border-brand-100 flex items-center justify-center text-sm font-bold text-brand-600 shadow-sm">E</div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-gray-800 truncate">Eng. Mohamed</p>
            <p className="text-[11px] text-gray-400 truncate">admin@sanadak.com</p>
          </div>
        </div>
      </div>
    </aside>
