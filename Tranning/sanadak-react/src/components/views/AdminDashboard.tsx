...
export default function AdminDashboard() {
  const { instructors, setInstructors, showToast, pushToUndoStack } = useApp();
  ...
  const deleteIns = (idx: number) => {
    ...
    showToast(`تم حذف المحاضر ${deletedItem.name} • [تراجع]`, 'warning', true);
  };

  const saveInstructor = () => {
    const amountNum = parseInt(formAmount.trim(), 10);
    if (!formName.trim() || !formCourse.trim() || isNaN(amountNum)) {
      showToast('الرجاء إدخال بيانات صحيحة في جميع الحقول الإلزامية', 'warning');
      return;
    }
    ...
    if (editIndex !== null) {
      ...
      pushToUndoStack(() => setInstructors(oldList));
      showToast('تم تعديل بيانات المحاضر بنجاح • [تراجع]', 'success', true);
    } else {
      setInstructors(prev => [...prev, updatedItem]);
      pushToUndoStack(() => setInstructors(oldList));
      showToast('تم إضافة محاضر جديد بنجاح ✓ • [تراجع]', 'success', true);
    }
  ...
  const updatePayoutModelChart = (model: PayoutModel) => {
    ...
    showToast(`تم تغيير وضع تحليل المدفوعات إلى: ${model}`, 'info');
  };

  const savePayoutSettings = () => {
    setChartData(paymentChartData[selectedPayoutModel]);
    showToast('تم تحديث وحفظ نظام الدفع والتحليلات بنجاح ✓', 'success');
  };
    ...
    <div className="space-y-6 animate-fade-in text-right" dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-all neon-border">
          <p className="text-gray-500 text-sm font-semibold mb-1">إجمالي المدفوعات</p>
          <p className="text-3xl font-black text-gray-900 neon-text-glow">EGP {totalPaid.toLocaleString()}</p>
          <p className="text-xs text-emerald-600 font-semibold mt-1">+18% من الشهر السابق</p>
        </div>
        
        {/* Card 2 */}
        <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-all neon-border">
          <p className="text-gray-500 text-sm font-semibold mb-1">الرصيد المعلق</p>
          <p className="text-3xl font-black text-amber-500">EGP {pendingPaid.toLocaleString()}</p>
          <p className="text-xs text-gray-400 font-semibold mt-1">المحاضرين المعلقة مستحقاتهم</p>
        </div>

        {/* Card 3 */}
        <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-all neon-border">
          <p className="text-gray-500 text-sm font-semibold mb-1">المحاضرين النشطين</p>
          <p className="text-3xl font-black text-gray-900">{instructors.length}</p>
          <p className="text-xs text-gray-400 font-semibold mt-1">في دورات معتمدة</p>
        </div>

        {/* Card 4 */}
        <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-all neon-border">
          <p className="text-gray-500 text-sm font-semibold mb-1">إجمالي الطلاب</p>
          <p className="text-3xl font-black text-gray-900">342</p>
          <p className="text-xs text-emerald-600 font-semibold mt-1">+24 هذا الأسبوع</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-gray-900 mb-1">نظام الدفع</h3>
            <p className="text-xs text-gray-400 mb-5">اختر نموذج الدفع للمحاضرين</p>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer hover:border-gray-300 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm text-gray-800">Per Student</p>
                    <p className="text-[11px] text-gray-400">لكل طالب مسجل</p>
                  </div>
                </div>
                <input
                  type="radio"
              ...
              <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer hover:border-gray-300 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm text-gray-800">Per Round</p>
                    <p className="text-[11px] text-gray-400">لكل دورة مكتملة</p>
                  </div>
                </div>
                <input
                  type="radio"
              ...
              <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer hover:border-gray-300 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm text-gray-800">Monthly</p>
                    <p className="text-[11px] text-gray-400">راتب شهري ثابت</p>
                  </div>
                </div>
                <input
                  type="radio"
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-bold text-gray-900">تحليل المدفوعات</h3>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="font-bold text-gray-900">سجل مدفوعات المحاضرين</h3>
          ...
          <table className="w-full text-sm text-zinc-700 text-center align-middle">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50 text-gray-400 text-xs font-bold uppercase tracking-wider">
                <th className="px-5 py-3">المحاضر</th>
            ...
            <tbody className="divide-y divide-gray-100 text-xs font-semibold text-gray-800">
              {instructors.map((ins, i) => {
                const statusClass = ins.status === 'تم الدفع' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600';
                ...
                return (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className={`${modelClass} px-2.5 py-1 rounded-lg text-xs font-semibold`}>{ins.model}</span>
                    </td>
                    <td className="px-5 py-4 font-bold text-gray-900">EGP {ins.amount.toLocaleString()}</td>
                    <td className="px-5 py-4">
                      <span className={`${statusClass} px-2.5 py-1.5 rounded-lg text-xs font-bold`}>{ins.status}</span>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => openEditModal(i)} className="w-8 h-8 rounded-full bg-gray-50 border border-gray-200 text-zinc-500 flex items-center justify-center hover:bg-gray-100 transition-colors shadow-sm" title="تعديل">
                        ...
                        <button onClick={() => deleteIns(i)} className="w-8 h-8 rounded-full bg-red-50 border border-red-100 text-brand-600 flex items-center justify-center hover:bg-red-100 transition-colors shadow-sm" title="حذف">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 animate-fade-in" style={{ backdropFilter: 'blur(8px)' }}>
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg mx-4 p-8 animate-slide-up shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">{editIndex !== null ? 'تعديل بيانات المحاضر' : 'إضافة محاضر جديد'}</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-500 mb-2">اسم المحاضر *</label>
                <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder="أدخل الاسم بالكامل" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-gray-900 outline-none focus:ring-2 focus:ring-brand-500 font-bold" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-500 mb-2">المادة / الدورة *</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-500 mb-2">نموذج الحساب / Payout Model</label>
                <select value={formModel} onChange={e => setFormModel(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-gray-900 outline-none focus:ring-2 focus:ring-brand-500 font-bold">
                  ...
                  <option>Per Round</option>
                  <option>Monthly</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-500 mb-2">القيمة / Amount (EGP) *</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-500 mb-2">الحالة</label>
