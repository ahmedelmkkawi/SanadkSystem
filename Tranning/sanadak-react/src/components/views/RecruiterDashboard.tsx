...
const softCategories = [
  ...
  { value: 'problemSolving', label: 'حل المشكلات' },
  { value: 'teamwork', label: 'العمل الجماعي' },
  { value: 'timeManagement', label: 'إدارة الوقت' },
  { value: 'adaptability', label: 'المرونة والتكيف' },
  { value: 'criticalThinking', label: 'التفكير النقدي' },
];

export default function RecruiterDashboard({ onSchedule }: RecruiterDashboardProps) {
  const { lang, applicants, setApplicants, showToast, pushToUndoStack } = useApp();
  ...
  const deleteApplicant = (idx: number) => {
    ...
    showToast(`تم حذف المرشح ${name} • [تراجع]`, 'warning', true);
  };
  ...
  const focusEvaluationForm = (filteredIdx: number) => {
    ...
    if (cand.status === 'evaluated' && cand.techScore) {
      showToast(`تعديل التقييم الحالي للمرشح: ${cand.name}`, 'info');
    } else {
      showToast(`بدء تقييم جديد للمرشح: ${cand.name}`, 'info');
    }
  ...
  const submitEval = () => {
    if (activeEvalIndex === null) { showToast('الرجاء اختيار مرشح لتقييمه من جدول المتقدمين أولاً', 'warning'); return; }
    if (!techScoreRef.current || !softScoreRef.current || selectedCatsRef.current.length === 0 || feedbackRef.current.trim().length < 10) {
      showToast('يرجى ملء جميع الحقول المطلوبة بالشكل الصحيح', 'warning'); return;
    }
    ...
    pushToUndoStack(() => setApplicants(oldList));
    showToast(`تم حفظ تقييم المرشح ${name} بنجاح! • [تراجع]`, 'success', true);
    cancelEval();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'إجمالي المتقدمين', val: '47', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', iconBg: 'bg-brand-50', iconColor: 'text-brand-500', sub: '+12% هذا الشهر', subColor: 'text-emerald-600' },
          { label: 'مقابلات مجدولة', val: '12', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z', iconBg: 'bg-blue-50', iconColor: 'text-blue-500', sub: 'بانتظار المراجعة', subColor: 'text-gray-400' },
          { label: 'تم القبول', val: '8', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', iconBg: 'bg-emerald-50', iconColor: 'text-emerald-500', sub: '17% نسبة القبول', subColor: 'text-emerald-600' },
          { label: 'تم الرفض', val: '27', icon: 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z', iconBg: 'bg-red-50', iconColor: 'text-red-500', sub: 'آخر تحديث: اليوم', subColor: 'text-gray-400' },
        ].map(s => (
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-white border border-gray-200 rounded-2xl shadow-sm">
          <div className="relative w-full sm:w-80">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} type="text" placeholder="البحث عن طريق اسم المتقدم أو الخبرة..." className="w-full bg-gray-50 border border-gray-200 rounded-xl ps-11 pe-4 py-2.5 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-brand-500 placeholder-gray-400 font-medium" />
          </div>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 font-bold outline-none focus:ring-2 focus:ring-brand-500">
            ...
            <option value="accepted">مقبول مبدئياً</option>
            ...
            <table className="w-full text-center align-middle">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50 text-gray-400 text-xs font-bold uppercase tracking-wider">
                  <th className="px-6 py-4">اسم المرشح</th>
                  <th className="px-6 py-4">العمر</th>
                  <th className="px-6 py-4">الخبرة</th>
                  ...
                  <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-5">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-gray-400 mt-1">تاريخ التقديم: {a.date}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-center gap-2.5">
                        {a.actionType === 'quick-actions' && (
                          <button onClick={() => onSchedule(a.name)} className="w-8 h-8 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-center justify-center hover:bg-emerald-100 transition-colors shadow-sm animate-pulse" title="قبول وجدولة مقابلة">
                        ...
                        {a.actionType === 'eval-btn' && (
                          <button onClick={() => focusEvaluationForm(i)} className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all shadow-md shadow-brand-500/20 flex items-center gap-1.5">
                            ...
                            <span>تقييم المقابلة</span>
                          </button>
                        )}
                        {a.actionType === 'score-display' && (
                          <span className="text-xs text-brand-600 font-extrabold">التقييم: {a.score}</span>
                        )}
                        <button onClick={() => deleteApplicant(i)} className="w-8 h-8 rounded-full bg-red-50 border border-red-100 text-brand-600 flex items-center justify-center hover:bg-red-100 transition-colors shadow-sm" title="حذف">
      ...
      <div id="evalFormSection" className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-gray-100">
          <h3 className="font-bold text-lg text-gray-900">نموذج تقييم المقابلة</h3>
          <p className="text-sm text-gray-400 mt-1">تقييم شامل للمرشح بناءً على الأداء التقني والمهارات الشخصية</p>
        </div>
        ...
        {activeEvalIndex !== null && (
          <div className="mx-5 mt-4 p-4 bg-brand-50 border border-brand-100 rounded-xl flex items-center justify-between animate-fade-in">
            <span className="text-sm font-bold text-brand-900">
              اسم المرشح قيد التقييم حالياً: <span className="underline text-brand-600 font-extrabold">{applicants[activeEvalIndex]?.name}</span>
            </span>
            <button onClick={cancelEval} className="text-xs text-red-600 hover:text-red-700 font-black transition-colors">إلغاء التحديد ✗</button>
          </div>
        )}
        ...
        <form key={evalKey} className="p-5 space-y-6" onSubmit={e => { e.preventDefault(); submitEval(); }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Tech Score */}
            <div>
              <label className="block text-sm font-semibold text-gray-500 mb-3">التقييم التقني (1-5)</label>
            ...
            {/* Soft Score */}
            <div>
              <label className="block text-sm font-semibold text-gray-500 mb-3">المهارات الشخصية (1-5)</label>
              <div className="flex gap-2">
                {[1,2,3,4,5].map(n => (
                  <label key={n} className="flex-1 text-center py-3 rounded-lg border border-gray-200 bg-gray-50 cursor-pointer hover:border-brand-400 font-bold transition-all text-gray-600 has-[:checked]:bg-brand-600 has-[:checked]:border-brand-600 has-[:checked]:text-white">
          ...
          {/* Categories */}
          <div>
            <label className="block text-sm font-semibold text-gray-500 mb-3">التصنيفات المجتازة (المهارات الشخصية والناعمة)</label>
          ...
          {/* Feedback */}
          <div>
            <label className="block text-sm font-semibold text-gray-500 mb-2">ملاحظات التقييم</label>
            <textarea defaultValue={feedbackRef.current} onChange={e => feedbackRef.current = e.target.value} rows={4} placeholder="اكتب ملاحظات مفصلة عن أداء المرشح..." className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-gray-900 outline-none focus:ring-2 focus:ring-brand-500 resize-none placeholder-gray-400 text-sm" />
          </div>

          {/* Decision */}
          <div>
            <label className="block text-sm font-semibold text-gray-500 mb-3">القرار النهائي</label>
            <div className="flex gap-3">
              {[{ val: 'pass', label: 'ناجح ✓', checked: 'bg-emerald-50 border-emerald-500 text-emerald-700' }, { val: 'review', label: 'يحتاج مراجعة ⟳', checked: 'bg-amber-50 border-amber-500 text-amber-700' }, { val: 'fail', label: 'غير ناجح ✗', checked: 'bg-red-50 border-red-500 text-red-700' }].map(d => (
